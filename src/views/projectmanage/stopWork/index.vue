<template>
  <div class="app-container">
    <el-tabs v-model="queryParams.status" @tab-click="handleQuery()">
      <el-tab-pane label="停工管理" name="T"></el-tab-pane>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            size="mini"
            type="text"
            @click="goStopWorkDetail(scope.row)"
            >查看</el-button>
          <el-button
            size="mini"
            type="text"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        this.loading = true;
        projectInfo.stopWorkSelectAllPage(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
      ...
      deleteStopWork(){
        ...
        this.$confirm('是否确认删除?', "警告", {
          ...
          return projectInfo.deleteStopWork(this.ids);
        }).then(() => {
          this.getList();
      ...
      goStopWorkForm(row){
        if (row) {
          this.$router.push({ path: '/projectmanage/stopWorkForm', query: { id: row.id,status:this.queryParams.status,isDisabledForm:'false' } })
        } else {
          this.$router.push({ path: '/projectmanage/stopWorkForm' })
        }
      },
      goStopWorkDetail(row){
        this.$router.push({ path: '/projectmanage/stopWorkForm', query: { id:row.id ,isDisabledForm:'true'} })
      },
      //选择多选框触发
