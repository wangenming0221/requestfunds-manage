<template>
  <div class="app-container">
    <el-form ref="stopWorkForm" :model="form" label-width="150px" :rules="rules">
      <div class="common-nav">
        <div class="common-nav_title">停工申请单</div>
        <div>
      ...
      <el-row :gutter="24" class="row-style">
        ...
        <el-col :span="8">
          <el-form-item prop="stopType" label="停工原因">
            <el-select v-model="form.stopType" placeholder="请选择停工原因" :disabled="isDisabledForm">
              <el-option v-for="item in stopWorkReasonOptions"
                         :key="item.name"
                         :label="item.name"
                         :value="item.id">
              </el-option>
        ...
        <el-col :span="24">
          <el-form-item prop="applyComment" label="停工申请描述">
            <el-input  type="textarea"  :rows="10"  placeholder="请输入停工申请描述"  v-model="form.applyComment" :disabled="isDisabledForm"></el-input>
          </el-form-item>
        </el-col>
        ...
        <el-col :span="12">
          <el-form-item prop="branchDirector" label="分公司工程负责人">
            <el-autocomplete
                clearable
                class="autocomplete"
                v-model="form.branchDirector"
                :fetch-suggestions="(queryString, cb) => querySearchAsyncBranch(queryString, cb, form.branchDirector)"
                placeholder="请选择分公司工程负责人"
        ...
        <el-col :span="8">
          <el-form-item prop="stopBuildingsCount" label="本次申请停工楼栋总数">
            <el-input value="1" placeholder="请输入本次申请停工楼栋总数" disabled/>
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item prop="totalBuildingNumber" label="合同范围内楼栋总数">
            <el-input v-model="form.totalBuildingNumber"  placeholder="请输入合同范围内楼栋总数" disabled/>
          </el-form-item>
        </el-col>
        <el-col :span="8">
        ...
        <el-col :span="24" class="common-title">项目索赔情况简述</el-col>
        <el-col :span="24">
          <el-form-item prop="isNeedIndemnity" label="项目停工是否发生索赔">
            <el-select v-model="form.isNeedIndemnity" placeholder="请选择项目停工是否发生索赔" :disabled="isDisabledForm">
              <el-option v-for="item in needIndemnityOptions"
                         :key="item.name"
                         :label="item.name"
                         :value="item.id">
              </el-option>
        ...
        <el-col :span="24">
          <el-form-item prop="indemnityItem" label="索赔事项有哪些">
            <el-input  type="textarea"  :rows="10"  placeholder="请输入索赔事项有哪些"  v-model="form.indemnityItem" :disabled="isDisabledForm"></el-input>
          </el-form-item>
        </el-col>


        <el-col :span="24" class="common-title">停工楼栋具体信息</el-col>
        <el-col :span="24" class="mb20">
    ...
    <!-- 新增停工楼栋具体信息 -->
    <el-dialog  title="停工楼栋"  :visible.sync="singleBuildingVisible"  width="800px" :before-close="cancelAddUserRelations">
      <div class="wrap">
        <el-table v-loading="loading" :data="singleBuildingList" border style="width: 750px;height:500px;overflow-y: auto;" >
          <el-table-column  width="60">

            <template slot-scope="scope">
              <el-checkbox v-model="scope.row.checked" :disabled="scope.row.disabled" @change="changeSingleBuilding(scope.row)"/>
            </template>
          </el-table-column>
  ...
  export default {
    ...
    data() {
      return {
        ...
        //停工原因
        stopWorkReasonOptions: projectmanagedic.stopWorkReasonOptions,
        //是否发生索赔
        needIndemnityOptions: projectmanagedic.needIndemnityOptions,
    ...
    methods: {
      ...
      getDetail(id){
        projectInfo.getStopWorkDetail({ id }).then(res => {
          this.form = {
            creator:store.getters.name,
            createTime:parseTime(new Date()),
            ...res.data.projectsStopWorkInfo,
      ...
      projectInfoById(projectMdmId){
        projectInfo.projectInfoById({ projectMdmId}).then(res => {
          ...
          if(this.$route.query.stopWorkId){
            this.getDetail(this.$route.query.stopWorkId);
          }
        })
      },
      //承包人列表
      ...
      verifySubmitForm() {
        if(!this.timer){
          this.$refs['stopWorkForm'].validate((valid) => {
            if (valid) {
